...
`Notices\Queue::render()` prints the queue and then clears it, and it is gated on the `activate_plugins`
...
anything. The value is an `array<string,string>` keyed `slug:type` — for example
`give-recurring:merge` — and the messages are plain text; the default rendering escapes them with
`esc_html()`, so a link in a message comes out as literal angle brackets.
