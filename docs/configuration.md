...
trace. A duplicate slug is found later, on `plugins_loaded`, since registrations are buffered
until the first read: it is refused there and reported through `_doing_it_wrong()`, the first
registration under the slug stands, and the second is discarded.
