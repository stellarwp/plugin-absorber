...
class Scheduler {
	...
	public function wire(): void {
		if ( is_admin() ) {
			...
			// the queue when they fire, so neither builds anything at boot, but a named callback
			// can also be taken back with remove_filter() -- which matters most here, on the one
			// hook that rewrites a screen WordPress drew rather than adding one of our own. The
			// closures below are shaped that way for a reason these two do not share: the
			// plugins_loaded sequence has to be runnable inline as well as wirable.
			add_filter( 'wp_admin_notice_markup', [ Loader::class, 'filter_activation_error_markup' ] );
		}
